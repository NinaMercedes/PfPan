...
hist = read_csv('node.hist', sep='\t', header=[0,1], index_col=0)
hist.columns = ['node']
...
# ---- load growth ----
growth = read_csv('node.growth', sep='\t', header=list(range(N_HEADERS)), index_col=0)
...
ax = axs[0]
ax.bar(hist.index.astype(str), hist['node'], color=CUSTOM_COLORS[0])
...
ax.set_title('A: Node coverage distribution (#nodes)', fontsize=14, loc='left', fontweight='bold')
ax.set_ylabel('#nodes')
...
ax.set_yticks(yticks)
ax.set_yticklabels(calibrate_yticks_text(yticks))
ax.set_title('B: Cumulative pangenome growth (#nodes)', fontsize=14, loc='left', fontweight='bold')
ax.set_ylabel('#nodes')
...
ax.set_yticks(yticks)
ax.set_yticklabels(calibrate_yticks_text(yticks))
ax.set_title('C: Novel sequence per additional genome (#nodes)', fontsize=14, loc='left', fontweight='bold')
ax.set_ylabel('#nodes')
ax.set_xlabel('genomes added')
ax.legend(loc='upper right', fontsize=9)

plt.tight_layout()
plt.savefig('/mnt/user-data/outputs/Figure1_node_FINAL.png', dpi=200)
plt.savefig('/mnt/user-data/outputs/Figure1_node_FINAL.pdf')
print('done')
